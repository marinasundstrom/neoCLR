...
fn allocation_budget_counts_live_objects_and_preserves_identities() {
    ...
    let result = program("", &body, "Int32&").run(limits(1)).unwrap();
    let Value::SlotReference(reference) = result.value else {
        panic!("expected reference")
    };
    let index = reference.allocation_id().unwrap();
    assert_eq!(index, 100);
...
fn caller_cells_and_byref_targets_survive_callee_collection() {
    let extra = ".function Read(Int32& item) -> Int32\nldc.i4 1\nheap.new\npop\nldc.i4 2\nheap.new\npop\nldarg item\nldobj Int32\nret\n.end";
    let result = program(
        extra,
        ".local Int32& item\nldc.i4 42\nheap.new\nstloc item\nldloc item\ncall Read(Int32&)",
        "Int32",
...
fn returned_cycles_remain_reachable_without_recursive_marking() {
    let result = program(CYCLE, "call Cycle()", "System.Value&")
        .run(limits(1))
        .unwrap();
    assert_eq!(result.heap.len(), 1);
    assert_eq!(
...
#[test]
fn collection_history_is_bounded_and_reports_root_survival() {
    let body = format!(
        "{}ldc.i4 42\nheap.new",
        "ldc.i4 0\nheap.new\npop\n".repeat(100)
    );
    let result = program("", &body, "Int32&").run(limits(1)).unwrap();
    let events: Vec<_> = result.heap.collection_events().copied().collect();
    assert_eq!(events.len(), 64);
    assert_eq!(events[0].sequence, 38);
    let last = events.last().unwrap();
    assert_eq!(last.reason, neoclr::CollectionReason::ExecutionCompleted);
    assert_eq!(
        (last.roots, last.before, last.after, last.reclaimed),
        (1, 1, 1, 0)
    );
}
