...
## Common verb checkpoint — 2026-09-25

HttpClient now adds Put, Patch and Delete with string/Uri and optional token overloads;
HttpRequest has corresponding absolute-address factories. All use the existing Send
pipeline and BaseUri rules. Pre-cancellation skips address parsing and dispatch. PUT
and PATCH reuse buffered content with generated byte Content-Length; DELETE creates
empty content. Non-success responses remain inspectable data. The socket provider
rejects a nonempty GET/DELETE body before DNS, and the server rejects their nonzero
request Content-Length. This is a POC policy, not a blanket statement about HTTP.

Primary .NET comparison reviewed 2026-09-25:
[PutAsync](https://learn.microsoft.com/en-us/dotnet/api/system.net.http.httpclient.putasync?view=net-10.0),
[PatchAsync](https://learn.microsoft.com/en-us/dotnet/api/system.net.http.httpclient.patchasync?view=net-10.0)
and [DeleteAsync](https://learn.microsoft.com/en-us/dotnet/api/system.net.http.httpclient.deleteasync?view=net-10.0)
provide string/Uri and cancellation overloads; PUT/PATCH take content and the ordinary
DELETE helper does not. neoCLR adopts that familiar division with Result-based errors,
its existing strict BaseUri policy, fixed buffers and no automatic retry/redirect.
PATCH accepts opaque content; applications choose the patch media type and implement
its semantics. Method helpers do not imply a routing, storage or patch engine.

This additive surface retains the current string Method property; it does not introduce
an HttpMethod enum or unconstrained method constructor. HEAD requires request-aware
response framing and remains in the framing slice; OPTIONS and extension methods
