...
| Delegates and lambdas | [Delegates as the shared runtime callable abstraction](delegates.md); language function values and lambdas build on them, with bound receivers and captured environments | Invocation signatures; capture by value versus reference; escaping captures and GC roots; delegate equality and possible multicast behavior; whether low-level function pointers are needed |
