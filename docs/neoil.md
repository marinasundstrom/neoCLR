# neoIL assembler reference, format 1
...
- `.module Name` and `.entry FunctionName` each occur exactly once.
- `.type Name` begins a record; `.field Name Type` declares ordered fields;
  `.end` closes the type.
- `.function Name -> Type` begins a free function; `.param Type` and `.local Type`
  declare slots before any instructions or labels; `.end` closes the function.
...
The entry function takes no arguments and may return any supported type.
...
| `call Name` | `P0,…,Pn → R` | Call free function or bootstrap intrinsic |
...
The console intrinsic appends to a host output buffer; the CLI writes that buffer
on successful execution and handles host output failures separately. These three
symbols are reserved and cannot be redefined by a module.
...
and `body`; no owning class is needed. Instruction objects use `op` and optional
