...
| 5. Generic constraints | The initial [notvoid/notreference subset](generic-constraints.md) is implemented in metadata, runtime checks and Neo. Next add base/interface contracts with constrained-member lookup; notnull requires nullable metadata. Extend symbolic implication checks and reflection as needed. | A small generic API accepts valid arguments and rejects invalid ones consistently from source and IL/artifacts. |
