...
| 5. Generic constraints | [Generic constraints](generic-constraints.md) now cover notvoid/notreference and nominal base/interface bounds, with Neo member calls and constrained reference views through T&. Addressable bare-T calls work with notreference; next assess general receiver adaptation; notnull requires nullable metadata. Extend symbolic implication checks and reflection as needed. | A small generic API accepts valid arguments and rejects invalid ones consistently from source and IL/artifacts. |
