...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='neoclr-json-dom-') as folder:
        root = Path(folder)
        ...
        shutil.copyfile(here.parent / 'json-streams/Reference.cs', reference / 'Program.cs')
        ...
        project.mkdir()
        for name in ('Main.rvn', 'Sample.rvn', 'JsonDom.rvnproj'):
            shutil.copyfile(here / name, project / name)
        built = subprocess.run(['dotnet', 'msbuild', str(project / 'JsonDom.rvnproj'), '-nologo', '-v:minimal'], env=env, capture_output=True, text=True, timeout=240)
