...
The main fixture checks separate parsed objects, typed and Equatable<Path> equality,
...
rehashing. Equatable<int> exercises a value type without Nullable<int>.

Equatable<T>.Equals accepts T, not T?. Path's typed overload accepts Path.
...
Equatable<Path>.Equals despite the non-nullable T contract (including explicit
