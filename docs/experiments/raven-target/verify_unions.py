"""Check generic union case faults and invalid payload/carrier defaults."""
import argparse
...
with tempfile.TemporaryDirectory(prefix='neoclr-union-values-') as temporary:
    root = Path(temporary)
...
''')
    ...
    (root / 'Main.rvn').write_text('''import System.*
func Main() {
    let result = default(Option<string>)
    result.GetSomeCase()
}
''')
    result = subprocess.run(command, capture_output=True, text=True, timeout=90)
