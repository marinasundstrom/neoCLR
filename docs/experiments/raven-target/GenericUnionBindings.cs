...
static class GenericUnionBindings
{
    ...
    static string? Map(TypeReference type, int depth)
    {
        ...
        return EnumBindings.Type(type) ?? ApplicationTypes.Type(type) ?? WorkerBindings.Type(type) ?? TaskBindings.Type(type) ?? AsyncBindings.Type(type) ?? ReflectionBindings.Type(type) ?? CollectionBindings.Type(type) ?? InterfaceBindings.Type(type) ?? DelegateBindings.Type(type) ?? NativeMemoryBindings.Type(type) ?? ManagedArrayBindings.Type(type) ?? CalendarBindings.Type(type) ?? ErrorBindings.Type(type) ?? PrimitiveBindings.Type(type) ?? type.MetadataType switch {
