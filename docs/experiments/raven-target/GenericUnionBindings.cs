...
static class GenericUnionBindings
{
    ...
    static string? Map(TypeReference type, int depth)
    {
        ...
        return ApplicationTypes.Type(type) ?? ReflectionBindings.Type(type) ?? CollectionBindings.Type(type) ?? InterfaceBindings.Type(type) ?? DelegateBindings.Type(type) ?? NativeArrayBindings.Type(type) ?? ManagedArrayBindings.Type(type) ?? CalendarBindings.Type(type) ?? ErrorBindings.Type(type) ?? PrimitiveBindings.Type(type) ?? type.MetadataType switch {
