...
static class ManagedArrayBindings
{
    ...
    public static bool IsReference(string type) => HttpBindings.IsName(type) || SocketBindings.IsName(type) || (IPAddressBindings.IsName(type) || UriBindings.IsName(type)) || PathBindings.IsName(type) || AsyncBindings.IsType(type) || ReaderBindings.IsName(type) || type == FileSystemBindings.Name || StorageItemBindings.IsName(type) || ApplicationTypes.IsReference(type) || type == "String" || type == "System.Object" || IsType(type)
