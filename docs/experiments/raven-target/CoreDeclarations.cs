...
static class CoreDeclarations
{
    ...
    public static void Write(string path, bool includeConsole = true, bool stringParameter = true, bool unionProbe = false, bool collectionProbe = false, bool libraryBootstrap = false)
    {
        ...
        if (collectionProbe) declarations += JsonBindings.Declarations + HttpBindings.Declarations + SocketBindings.Declarations + CancellationBindings.Declarations + WorkerBindings.Declarations + TaskBindings.ForReference(libraryBootstrap) + Utf8Bindings.Declarations + UnicodeScalarBindings.Declarations + QueryBindings.Declarations + OutcomeOperatorBindings.Declarations + CollectionDeclarations.Source + ReflectionBindings.Declarations + ReflectionBindings.ProviderDeclarations + NativeMemoryBindings.Declaration + InterfaceBindings.Declarations;
