...
with tempfile.TemporaryDirectory(prefix='neoclr-project-check-') as temporary:
    ...
    cases = [('ArrayTour', 'library-array-tour.rvn', (bridge / 'samples/library-array-tour.expected.txt').read_text()),
             ('Utf8', 'library-utf8.rvn', (bridge / 'samples/library-utf8.expected.txt').read_text()),
             ...
             ('GraphemeStrings', 'library-grapheme-strings.rvn', (bridge / 'samples/library-grapheme-strings.expected.txt').read_text()),
    ...
    for label, source, diagnostic in [
        *[(f'Missing{case}Match', (bridge / 'samples/library-introspection-interfaces.rvn').read_text()
        ...
        ('PathArgumentMismatch', 'func Main() { System.Storage.Path.Combine(42, 7) }', 'RAV'),
        ('PathUnsupportedApi', 'func Main() { System.Storage.Path.GetFullPath(".") }', 'RAV'),
