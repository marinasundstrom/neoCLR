...
static class LibraryImplementation
{
    ...
    {
        if (StorageProviderBindings.SameType(left, right) || PathBindings.SameType(left, right) || StreamBindings.SameType(left, right) || WorkerBindings.SameType(left, right) || AsyncBindings.SameType(left, right) || TaskBindings.SameType(left, right) || DescriptorLibrary.SameType(left, right)) return true;
