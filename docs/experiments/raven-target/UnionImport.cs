...
static class UnionImport
{
    ...
    static void WriteImplementation(string application, string core, string destination, bool collectionProfile, string[] dependencies, string? libraryOwner)
    {
        ...
        while (pending.TryDequeue(out var method))
        {
            ...
            if (locals.Any(t => !(libraryOwner is not null && t is "Value" or ParameterSnapshotBindings.Vector) && !(libraryOwner is not null && method.GenericParameters.Concat(method.DeclaringType.GenericParameters).Any(p => t == "T" + p.Position)) && !(((StorageItemBindings.IsName(t) || (UriBindings.IsName(t) || PathBindings.IsName(t))) || StreamBindings.IsName(t)) || ((HttpBindings.IsName(t) || SocketBindings.IsName(t)) || WorkerBindings.IsName(t))) && !ReaderBindings.IsName(t) && !EnumBindings.IsType(t) && !AsyncBindings.IsType(t) && !TaskBindings.IsType(t) && !ApplicationTypes.IsType(t) && !ManagedArrayBindings.IsType(t) && t != "System.Object" && !InterfaceBindings.IsInterface(t) && !NativeMemoryBindings.IsPointer(t) && !ReflectionBindings.IsType(t) && t != "arrayref<String>" && !DelegateBindings.IsType(t) && !GenericUnionBindings.IsType(t) && !CalendarBindings.IsReference(t) && !CalendarBindings.Types.Contains(t) && t != HashCodeBindings.Owner && !PrimitiveBindings.Types.Contains(t) && !ResultBindings.IsType(t) && !CollectionBindings.IsReference(t) && t is not ("Boolean" or "Int32" or "Double" or "String" or IntArray or Carrier or Ok or Error or Option or Some or None or VoidOption or VoidSome or Overflow or "Void" or VoidResult or VoidOk)))
            ...
            while (work.TryDequeue(out var index))
            {
                ...
                Slot ConvertTop(string type)
                {
                    if (stack.Count > 0 && stack[^1].Type == "FaultNull"
                        && ((((StorageItemBindings.IsName(type) || (UriBindings.IsName(type) || PathBindings.IsName(type))) || StreamBindings.IsName(type)) || ((HttpBindings.IsName(type) || SocketBindings.IsName(type)) || WorkerBindings.IsName(type))) || TaskBindings.IsType(type) || AsyncBindings.IsType(type) || type == "System.Object" || ApplicationTypes.IsReference(type)))
                    {
                        var nullValue = Pop();
            ...
            if (method.Body.InitLocals)
                for (var n = 0; n < locals.Length; n++)
                {
                    ...
                    if ((((StorageItemBindings.IsName(locals[n]) || (UriBindings.IsName(locals[n]) || PathBindings.IsName(locals[n]))) || StreamBindings.IsName(locals[n])) || ((HttpBindings.IsName(locals[n]) || SocketBindings.IsName(locals[n])) || WorkerBindings.IsName(locals[n]))) || TaskBindings.IsType(locals[n]) || AsyncBindings.IsType(locals[n]) || ApplicationTypes.IsType(locals[n]) || ErrorBindings.IsStandard(locals[n]) || ReflectionBindings.IsType(locals[n]) && locals[n] != "System.RuntimeTypeHandle" || ManagedArrayBindings.IsType(locals[n]) || CollectionBindings.IsReference(locals[n]) || (CalendarBindings.Types.Contains(locals[n]) || locals[n] == HashCodeBindings.Owner) || GenericUnionBindings.IsType(locals[n]) && !GenericUnionBindings.RequiresInitialization(locals[n])) output.AppendLine($"ldloca local{n}\ninitobj {locals[n]}");
