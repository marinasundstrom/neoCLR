            ...
            while (work.TryDequeue(out var index))
            {
                ...
                Slot ConvertTop(string type)
                {
                    if (stack.Count > 0 && stack[^1].Type == "FaultNull"
                        && ((((StorageItemBindings.IsName(type) || PathBindings.IsName(type)) || StreamBindings.IsName(type)) || (SocketBindings.IsName(type) || WorkerBindings.IsName(type))) || TaskBindings.IsType(type) || AsyncBindings.IsType(type) || type == "System.Object" || ApplicationTypes.IsReference(type)))
                    {
                        var nullValue = Pop();
