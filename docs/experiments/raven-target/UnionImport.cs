            ...
            while (work.TryDequeue(out var index))
            {
                ...
                Slot ConvertTop(string type)
                {
                    if (stack.Count > 0 && stack[^1].Type == "FaultNull"
                        && (((PathBindings.IsName(type) || StreamBindings.IsName(type)) || WorkerBindings.IsName(type)) || TaskBindings.IsType(type) || AsyncBindings.IsType(type) || ApplicationTypes.IsReference(type)))
                    {
                        var nullValue = Pop();
