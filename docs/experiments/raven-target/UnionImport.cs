...
static class UnionImport
{
    ...
    static void WriteImplementation(string application, string core, string destination, bool collectionProfile, string[] dependencies, string? libraryOwner)
    {
        ...
        if (libraryOwner is null) {
            ...
            IPAddressBindings.RejectExternalBranches(guestLibraries.Append(app.MainModule));
            JsonBindings.RejectExternalBranches(guestLibraries.Append(app.MainModule));
        }
        ErrorBindings.Reset(library.MainModule);
        ...
        while (pending.TryDequeue(out var method))
        {
            ...
            while (work.TryDequeue(out var index))
            {
                ...
                Slot ConvertTop(string type)
                {
                    if (stack.Count > 0 && stack[^1].Type == "FaultNull"
                        && ((((StorageItemBindings.IsName(type) || ((IPAddressBindings.IsName(type) || UriBindings.IsName(type)) || PathBindings.IsName(type))) || StreamBindings.IsName(type)) || (((JsonBindings.IsName(type) || HttpBindings.IsName(type)) || SocketBindings.IsName(type)) || (CancellationBindings.IsReference(type) || WorkerBindings.IsName(type)))) || TaskBindings.IsType(type) || AsyncBindings.IsType(type) || type == "System.Object" || ApplicationTypes.IsReference(type)))
                    {
                        var nullValue = Pop();
                ...
                switch (instruction.OpCode.Code)
                {
                    ...
                    case Code.Initobj:
                        ...
                        if (address.Local < 0)
                        {
                                ...
                                || method.DeclaringType.DeclaringType is { } unionOwner && StandardUnionLibrary.IsCandidate(unionOwner)))
                            {
                                ...
                                // replacing that receiver or weakening construction verification.
                                code.AppendLine("pop");
                                for (var fieldIndex = 0; fieldIndex < method.DeclaringType.Fields.Count; fieldIndex++)
                                {
                                    var initializedField = ApplicationTypes.Field(method.DeclaringType.Fields[fieldIndex], method, ProfileType)
                                        ?? throw new InvalidDataException("Missing union initialization field.");
                                    var initializedLocal = $"unionDefault{index}_{fieldIndex}";
                                    code.AppendLine($".local {initializedField.Type} {initializedLocal}\nldloca {initializedLocal}\ninitobj {initializedField.Type}\nldarg 0\nldloc {initializedLocal}\nstfld {initializedField.Owner}::{initializedField.Name}\npop");
                                }
                            }
                    ...
                    case Code.Ldarga: case Code.Ldarga_S:
                        ...
                        if (parameter < 0 || parameter >= args.Length || !(ApplicationTypes.IsType(args[parameter]) || EnumBindings.IsType(args[parameter]) || PrimitiveBindings.IsReceiver(args[parameter]) || ((args[parameter] == CancellationBindings.Token || CalendarBindings.Types.Contains(args[parameter])) || args[parameter] == HashCodeBindings.Owner) || ErrorBindings.IsType(args[parameter]) || GenericUnionBindings.IsType(args[parameter])))
                    ...
                    case Code.Callvirt:
                        ...
                        else if (targetMethod.Module == library.MainModule)
                        {
                            ...
                            var pathCall = HashCodeBindings.Bind(reference, targetMethod) ?? IPAddressBindings.Bind(reference, targetMethod) ?? UriBindings.Bind(reference, targetMethod) ?? PathBindings.Bind(reference, targetMethod);
                            ...
                            else
                            {
                                ...
                                var taskBinding = collectionProfile ? AsyncBindings.BindByRef(reference, targetMethod, index.ToString(), t => ProfileType(t)) ?? JsonBindings.Bind(reference, targetMethod, false, libraryOwner is not null) ?? HttpBindings.Bind(reference, targetMethod, false, libraryOwner is not null) ?? ReaderBindings.Bind(reference, targetMethod, false) ?? FileSystemBindings.Bind(reference, targetMethod, false) ?? StorageItemBindings.Bind(reference, targetMethod, false) ?? StreamBindings.Bind(reference, targetMethod, false, libraryOwner is not null) ?? SocketBindings.Bind(reference, targetMethod, false, libraryOwner is not null) ?? CancellationBindings.Bind(reference, targetMethod, false, libraryOwner is not null) ?? WorkerBindings.Bind(reference, targetMethod, false, libraryOwner is not null) ?? AsyncBindings.Bind(reference, targetMethod, false) ?? TaskBindings.Bind(reference, targetMethod, false, libraryOwner is not null) : null;
                        ...
                        for (var n = call.Arguments.Length - 1; n >= 0; n--)
                        {
                            ...
                            if (argument.Type.EndsWith('&'))
                            {
                                if (argument.Argument >= 0)
                                {
                                    ...
                                    if (n != 0 || !reference.HasThis || !(ApplicationTypes.Type(reference.DeclaringType) is not null || PrimitiveBindings.IsReceiver(reference.DeclaringType.Name) || ((reference.DeclaringType.FullName == CancellationBindings.Token || CalendarBindings.Types.Contains(reference.DeclaringType.FullName)) || HashCodeBindings.Type(reference.DeclaringType) is not null) || ErrorBindings.IsType(reference.DeclaringType.FullName.Replace('/', '.')) || GenericUnionBindings.IsType(GenericUnionBindings.Type(reference.DeclaringType) ?? "")))
            ...
            if (method.Body.InitLocals)
                ...
                {
                    if (ApplicationTypes.LibraryUnionRequiresInitialization(locals[n]) || !ErrorBindings.IsStandard(locals[n]) && ErrorBindings.Cases.TryGetValue(locals[n], out var errorCases) && errorCases.Length > 0) continue;
                    if ((((StorageItemBindings.IsName(locals[n]) || ((IPAddressBindings.IsName(locals[n]) || UriBindings.IsName(locals[n])) || PathBindings.IsName(locals[n]))) || StreamBindings.IsName(locals[n])) || (((JsonBindings.IsName(locals[n]) || HttpBindings.IsName(locals[n])) || SocketBindings.IsName(locals[n])) || (CancellationBindings.IsReference(locals[n]) || WorkerBindings.IsName(locals[n])))) || TaskBindings.IsType(locals[n]) || AsyncBindings.IsType(locals[n]) || ApplicationTypes.IsType(locals[n]) || ErrorBindings.IsStandard(locals[n]) || ReflectionBindings.IsType(locals[n]) && locals[n] != "System.RuntimeTypeHandle" || ManagedArrayBindings.IsType(locals[n]) || CollectionBindings.IsReference(locals[n]) || ((locals[n] == CancellationBindings.Token || CalendarBindings.Types.Contains(locals[n])) || locals[n] == HashCodeBindings.Owner) || GenericUnionBindings.IsType(locals[n]) && !GenericUnionBindings.RequiresInitialization(locals[n])) output.AppendLine($"ldloca local{n}\ninitobj {locals[n]}");
