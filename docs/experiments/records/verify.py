...
with tempfile.TemporaryDirectory(prefix='neoclr-records-') as folder:
    ...
    for declaration in ('record class Unsupported(Name: string?)', 'record class Unsupported(Value: object)'):
    ...
    for declaration in ('record class Unsupported(Value: int?)', 'record struct Point(X: int)\nrecord class Unsupported(Value: Point?)', 'func Invalid(value: int?) { }'):
        (root / 'Main.rvn').write_text('import System.*\n' + declaration + '\nfunc Main() { }\n')
        rejected = subprocess.run(['dotnet', 'msbuild', str(root / 'Records.rvnproj'), '-nologo', '-v:minimal'], env=env, capture_output=True, text=True, timeout=120)
        assert rejected.returncode != 0 and 'RAV0407' in rejected.stdout + rejected.stderr, rejected.stdout + rejected.stderr
