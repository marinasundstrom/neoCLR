...
2. **Use the selected member convention.** The [constructor/query contract](union-convention.md)
   ...
   name/arity distinction is implemented; nested ownership and case migration are next.
3. **Preserve the tested ordinary library carriers through migration.** System.Option and
   ...
   Int32.Parse and its checked-in callers are migrated; arithmetic, slicing, file and
   console API returns still use bootstrap carriers.
